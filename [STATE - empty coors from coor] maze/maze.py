...
def player_perspective(maze, player_coor, coors_viewed = []): # coors_viewed includes passed_by
    new_coors_found = 0
    empty_tiles_from_coor = {"x": player_coor[0], "y": player_coor[1], "LEFT": -1, "RIGHT": -1, "UP": -1, "DOWN": -1} # includes start/end, but not player
    
    coor = [player_coor[0], player_coor[1]]
    while maze[tuple(coor)] != 1: # left
        coors_viewed.append(tuple(coor))
        empty_tiles_from_coor["LEFT"] += 1
        coor[0] -= 1
    ...
    return coors_viewed, {coor: maze[coor] if coor in coors_viewed else 4 for coor in maze}, new_coors_found, empty_tiles_from_coor
